...
public abstract class Try<V> {
	...
	private static <V> Try<V> of(TryingSupplier<V> action) {
		try {
			return action.get();
		}
		catch (Exception e) {
	 ...
	 * @return the contained value, if available
	 */
	public abstract V getOrThrow(Function<? super Exception, ? extends Exception> exceptionTransformer);
	...
	@FunctionalInterface
	private interface TryingSupplier<V> {

		Try<V> get() throws Exception;
	...
	private static class Success<V> extends Try<V> {
		...
		@Override
		public V getOrThrow(Function<? super Exception, ? extends Exception> exceptionTransformer) {
			return this.value;
		}
	...
	private static class Failure<V> extends Try<V> {
		...
		@Override
		public V get() {
			return throwAs(this.cause);
		}

		@Override
		public V getOrThrow(Function<? super Exception, ? extends Exception> exceptionTransformer) {
			return throwAs(exceptionTransformer.apply(this.cause));
		}
		...
		// Cannot use ExceptionUtils due to package cycle
		@SuppressWarnings("unchecked")
		private <T extends Throwable> V throwAs(Throwable t) throws T {
			throw (T) t;
		}
	}
