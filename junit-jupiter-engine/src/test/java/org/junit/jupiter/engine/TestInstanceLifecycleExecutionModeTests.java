...
class TestInstanceLifecycleExecutionModeTests extends AbstractJupiterTestEngineTests {
	...
	void methodsInNestedTestClassesWithInstancePerClassInHierarchyHaveExecutionModeSameThread() {
		...
		var levelOneNestedClassDescriptor = firstChild(outerTestCaseClassDescriptor, NestedClassTestDescriptor.class);
		var levelOneTestMethodDescriptor = firstChild(levelOneNestedClassDescriptor, TestMethodTestDescriptor.class);
		var levelTwoNestedClassDescriptor = firstChild(levelOneNestedClassDescriptor, NestedClassTestDescriptor.class);
		var levelTwoTestMethodDescriptor = firstChild(levelTwoNestedClassDescriptor, TestMethodTestDescriptor.class);
		var levelThreeNestedClassDescriptor = firstChild(levelTwoNestedClassDescriptor,
			NestedClassTestDescriptor.class);
		var levelThreeTestMethodDescriptor = firstChild(levelThreeNestedClassDescriptor,
		...
		assertExecutionMode(levelOneNestedClassDescriptor, CONCURRENT);
		assertExecutionMode(levelOneTestMethodDescriptor, CONCURRENT);
		assertExecutionMode(levelTwoNestedClassDescriptor, CONCURRENT);
		assertExecutionMode(levelTwoTestMethodDescriptor, SAME_THREAD);
		assertExecutionMode(levelThreeNestedClassDescriptor, SAME_THREAD);
		assertExecutionMode(levelThreeTestMethodDescriptor, SAME_THREAD);
	}

	private static <T extends Node<?> & TestDescriptor> void assertExecutionMode(T testDescriptor,
			Node.ExecutionMode concurrent) {
		assertThat(testDescriptor.getExecutionMode()).describedAs("ExecutionMode for %s", testDescriptor).isEqualTo(
			concurrent);
	}
	...
	private <T extends Node<?> & TestDescriptor> T firstChild(TestDescriptor engineDescriptor,
		...
		return (T) engineDescriptor.getChildren().stream() //
				...
				.findFirst() //
				.orElseGet(() -> fail("No child with class " + testDescriptorClass + " found"));
	}
